<?php
function save_meta_box ($Membership) {
?>
...
function settings_meta_box ($Membership) {
?>
<p><input type="hidden" name="continuity" value="off" /><input type="checkbox" name="continuity" value="on" id="featured" tabindex="12" <?php if ($Membership->continuity == "on") echo ' checked="checked"'?> /><label for="featured"> <?php _e('Continued access' ,'Shopp'); ?></label></p>
<?php $roles = get_editable_roles(); ?>
<p><select name="role" id="wp-roles">
<?php foreach ($roles as $value => $role): $selected = (strtolower($Membership->role) == strtolower($value)?' selected="selected"':''); ?>
...
function sources_meta_box ($Membership) {
?>
<ul id="sources"></ul>
