<div class="wrap shopp">
	...
	<div class="tablenav">
		<div class="alignleft actions">
			...
			<button type="submit" id="update-button" name="update" value="order" class="button-secondary"><?php _e('Filter','Shopp'); ?></button>
			</span>
			</div>
	...
	<?php if (sizeof($Orders) > 0): ?>
		...
		<tr<?php if (!$even) echo " class='alternate'"; $even = !$even; ?>>
			...
			<td class="name column-name"><a class='row-title' href='<?php echo add_query_arg(array('page'=>$this->Admin->orders,'id'=>$Order->id),$Shopp->wpadminurl."admin.php"); ?>' title='<?php _e('View','Shopp'); ?> &quot;<?php echo $Order->id; ?>&quot;'><?php echo (empty($Order->firstname) && empty($Order->lastname))?"("._e('no contact name').")":"{$Order->firstname} {$Order->lastname}"; ?></a><?php echo !empty($Order->company)?"<br />$Order->company":""; ?></td>
...
var start = $('#start');
var startdate = false; match = false;
match = start.get(0).value.match(/^(\d{1,2}).{1}(\d{1,2}).{1}(\d{4})/);
if (match) startdate = new Date(match[3],(match[1]-1),match[2]);
...
var end = $('#end');
var enddate = false; match = false;
if (match) match = end.get(0).value.match(/^(\d{1,2}).{1}(\d{1,2}).{1}(\d{4})/);
enddate = new Date(match[3],(match[1]-1),match[2]);
