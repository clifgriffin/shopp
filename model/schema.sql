...
CREATE TABLE `shopp_price` (
	...
	`inventory` enum('off','on') default 'off',
	`sale` enum('off','on') default 'off',
	`shipping` enum('off','on') default 'on',
	`tax` enum('off','on') default 'on',
	`donation` enum('off','on') default 'off',
	`download` enum('off','on') default 'off',
	`created` datetime NOT NULL default '0000-00-00 00:00:00',
	`modified` datetime NOT NULL default '0000-00-00 00:00:00',
	PRIMARY KEY(`id`)
) ENGINE=MyIsAM DEFAULT CHARSET=utf8;
