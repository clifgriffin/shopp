...
class xHTMLvalidator {
	...
	var $_tags = array(
		...
		'area' => array(
			'attributes' => array(
				...
				'nohref' => '/^(true|false)$/',
				'ping', // html5
				'rel' => '/^(alternate|designates|stylesheet|start|next|prev|contents|index|glossary|copyright|chapter|section|subsection|appendix|help|bookmark| |shortcut|icon|tag)+$/', // html5
				'shape' => '/^(rect|rectangle|circ|circle|poly|polygon)$/',
				'type' // html5
			),
			'required' => array(
	...
	static $booleanattrs = array(
		'allowfullscreen', 'async', 'autofocus', 'checked', 'compact', 'declare', 'default', 'defer', 'disabled',
		'formnovalidate', 'hidden', 'inert', 'ismap', 'itemscope', 'multiple', 'multiple', 'muted', 'nohref',
    ...
    function validate(&$xhtml) {
        ...
        $xhtml_copy = $this->convertBooleanAttributes($xhtml_copy);
    ...
    function convertLiteralEntitiesToNumericalEntities($xhtml) {
        return str_replace(array_keys(self::$entities), array_values(self::$entities), $xhtml);
    }

    function convertBooleanAttributes($xhtml) {
		foreach ( self::$booleanattrs as $attr )
			str_replace("$attr ","$attr=\"\" ",$xhtml);
    }
    ...
    function getRules() {
        ...
        if (!isset($rules)) {
            ...
            $inline = array(
				'#pcdata', 'a', 'abbr', 'acronym', 'applet', 'b', 'basefont', 'bdo', 'big', 'br', 'button',
				'cite', 'code', 'dfn', 'em', 'font', 'i', 'img', 'input', 'kbd', 'label', 'map', 'object',
            ...
            $block = array(
				'address', 'blockcode', 'blockquote', 'center', 'dir', 'div', 'dl', 'fieldset', 'form',
				'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'handler', 'hr', 'iframe', 'isindex', 'menu', 'nl',
				'noframes', 'noscript', 'ol', 'p', 'pre', 'section', 'separator', 'table', 'ul'
            );
            ...
            $flow_tags = array('div', 'center', 'blockquote', 'noscript', 'dd', 'li', 'th', 'td');
            foreach($flow_tags as $flow_tag) {
                $rules[$flow_tag] = $flow;
            }
            $inline_tags = array(
				'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'dt', 'caption', 'legend',
				'tt', 'abbr', 'acronym', 'b', 'bdo', 'big', 'cite', 'code', 'dfn', 'em',
				'font', 'i', 'kbd', 'q', 's', 'samp', 'small', 'span', 'strike', 'strong',
				'sub', 'sup', 'u', 'var'
			);
    ...
    function cdata($parser, $cdata) {
        ...
        $previous = $this->_stack[count($this->_stack) -1];
        if ( $cdata != '' && in_array($previous, array('base', 'area', 'basefont', 'br', 'col', 'hr', 'img', 'input', 'link', 'meta', 'param')) ) {
