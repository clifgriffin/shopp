...
function Priceline (id,options,data,target,attachment) {
	...
	_.recurring = function (r) {
		...
		hd2 = $('<th><input type="hidden" name="'+fn+'[recurring][trial]" value="off" />'+
					...
					'<label for="trial-'+i+'"> '+TRIAL_LABEL+'111</label></th>').appendTo(headingsRow);
		...
		_.trialint = $('#trialint-'+i).change(function () {
			...
			_.trialperiod.val(s);
		}).val(r['trialint']).change();

		_.trial = hd2.find('#trial-'+i).attr('checked',(r['trial'] == "on"?true:false)).toggler(dis,ui,_.trialint);
		_.trialprice = $('#trialprice-'+i).val(r['trialprice']);

	};

	_.memberlevel = function (m) {
		var hd,ui,memberships,mo;
		...
		_.membership = $('#membership-'+i).val(m['membership']);
	};
	...
	_.Membership = function (data) {
		...
		_.saleprice(data.sale,data.saleprice);
		if (!tmp) _.recurring(data.recurring);
		if (!tmp) _.memberlevel(data.membership);
	};
...
function Priceline (id,options,data,target,attachment) {
	...
	_.recurring = function (d) {
		...
		_.interval = $('#interval-'+i).change(function () {
			...
			_.period.val(s);
		}).val(d['interval']).change();

		_.trialprice = $('#trialprice-'+i).val(asMoney(new Number(d['trialprice'])));
		_.trialperiod = $('#trialperiod-'+i).val(d['trialperiod']);
		_.trialint = $('#trialint-'+i).change(function () {
			var $this=$(this),s = _.trialperiod.val();
			if ($this.val() == 1) _.trialperiod.html(ps);
			else _.trialperiod.html(pp);
			_.trialperiod.val(s);
		}).val(d['trialint']).change();

		_.trial = hd2.find('#trial-'+i).attr('checked',(d['trial'] == "on"?true:false)).toggler(dis,ui,_.trialint);
	...
	_.memberlevel = function (d) {
		...
		_.membership = $('#membership-'+i).val(d);
