...
class ImageProcessor {
	...
	function scaleToFit($width,$height) {
		...
		$this->Processed = new StdClass();
		$this->Processed->width = $this->src->width;
		$this->Processed->height = $this->src->height;
		
		if ($this->src->width > $width || $this->src->height > $height) {		
				$aspect = $this->src->width / $this->src->height;  // Source Aspect Ratio
				$daspect = $width / $height; // Destination Aspect Ratio
				$diff = $aspect / $daspect; // Aspect difference Ratio
		
				if ($diff >= 1) { // Scale to width
					$scale = $width / $this->src->width;
					$this->Processed->width = $width;
					$this->Processed->height = ceil($this->src->height * $scale);
				...
				ImageCopyResampled($this->Processed->image, $this->src->image, 
					0, 0, 0, 0, 
					$this->Processed->width, $this->Processed->height, $this->src->width, $this->src->height);				
		} else { // Scaling unnecessary; the image fits
			$this->Processed->width = $this->src->width;
			$this->Processed->height = $this->src->height;
			$this->Processed->image = $this->src->image;
		}
	}
